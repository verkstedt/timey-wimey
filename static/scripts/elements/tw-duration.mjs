...
function partsToDateTime(parts) {
  return [
    'PT',
    ...Array.from(UNITS.keys()).map((unit) => `${parts.get(unit) || 0}${unit}`),
  ]
    .join('')
    .toUpperCase()
}
...
class TwDuration extends HTMLTimeElement {
  ...
  buildContent() {
    ...
    Array.from(UNITS).forEach(([unit, { title }]) => {
      ...
      unitElement.classList.add('a-duration__unit', `a-duration__unit--${unit}`)
  ...
  update(parts, context = this) {
    ...
    for (const [unit] of UNITS.keys()) {
      ...
      ignoredFromStart =
        unit === precision ? false : ignoredFromStart && value === 0
      context.querySelectorAll(`[data-unit=${unit}]`).forEach(setHidden)
  ...
  updateFromDateTime(context = this) {
    ...
    const dateTimeMatch = dateTime.match(/[0-9]+|[a-z]+/gi)
