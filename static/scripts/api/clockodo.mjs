...
class ApiClockodo {
  ...
  static mapProject({ internalCustomer, internalProject, internalService }) {
    return {
  ...
  destructProjectId(projectId) {
    const [internalProjectIdString, internalServiceIdString] =
      projectId.split('+')

    const internalProjectId = Number(internalProjectIdString)
    const internalServiceId = Number(internalServiceIdString)
    const internalCustomerId = this.getProjectCustomerId(internalProjectId)
  ...
  mapEntry(entry) {
    ...
    const internalCustomer = this.cache.internalCustomers.get(
      internalCustomerId
    ) || {
    ...
    const internalProject = this.cache.internalProjects.get(
      internalProjectId
    ) || {
      id: Number(internalProjectId),
      name: internalProjectName,
    }
    const internalService = this.cache.internalServices.get(
      internalServiceId
    ) || {
    ...
    if (data.projects) {
      Object.values(data.projects).forEach((rawProject) => {
        this.cache.internalProjects.set(
          ...
          this.constructor.mapInternalProject(rawProject)
        )
      })
    }
    if (data.services) {
    ...
    if (data.billable) {
      Object.entries(data.billable).forEach(([key, value]) => {
        ...
        if (keyParts.length === 1) {
          ...
          this.cache.internalCustomers.get(customerId).billable = billable
        } else if (keyParts.length === 2) {
          const projectId = Number(keyParts.pop())
          this.cache.internalProjects.get(projectId).billable = billable
        }
      })
    }
  }

  apiRequest = async (method, resource, { id = null, ...parameters } = {}) => {
  ...
  async fetchProjects() {
    ...
    internalServices.forEach((internalService) => {
      internalProjects.forEach((internalProject) => {
        const internalCustomer = internalCustomers.get(
          internalProject.customerId
        )
        projects.push(
  ...
  async start(projectId, taskName, { billable = null } = {}) {
    const { internalCustomerId, internalProjectId, internalServiceId } =
      this.destructProjectId(projectId)

    const internalProject = this.cache.internalProjects.get(internalProjectId)

    const billableFlag =
      billable == null ? internalProject.billableDefault : billable
  ...
  async update(
    id,
    { start = null, stop = null, project = null, task = null, billable = null }
  ) {
    const params = { id }
  ...
  getProjectCustomerId(projectId) {
    const project = this.cache.internalProjects.get(projectId)
    if (project === null) {
      throw new Error(`Failed to determine project id=${projectId}`)
    }

    const customer = this.cache.internalCustomers.get(project.customerId)
    if (customer === null) {
      throw new Error(
        `Failed to determine customer for a project id=${projectId}`
      )
    }
