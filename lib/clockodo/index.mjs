...
const apiRequest = async (method, resource, { id = null, ...parameters } = {}) => {
    let url = new URL(`${URL_BASE}/${resource}`);
    if (id !== null) {
        url.pathname += `/${id}`;
    }
    const options = {
...
function updateCache (data)
{
    if (data.projects)
    {
        Object.values(data.projects).forEach(rawCustomerOrProject => {
            ...
            else
            {
                ...
                cache.customers.set(Number(rawCustomer.id), mapCustomer(rawCustomer));
                rawProjects = rawCustomer.projects;
            }

            rawProjects.forEach(rawProject => {
                cache.projects.set(Number(rawProject.id), mapProject(rawProject));
            });
        });
    }
    if (data.services)
    {
        Object.values(data.services).forEach(rawService => {
            cache.taskTypes.set(Number(rawService.id), mapTaskType(rawService));
        });
    }
}
...
// FIXME Make fetchProjects and fetchTaskTypes independent of fetchCurrent
...
function mapCustomer (customer)
{
    ...
    return {
        id: Number(id),
        name,
        projectIds: new Set(customer.projects.map(project => Number(project.id))),
    };
}
